...
def KernelPriorEstimator( X_train, X_unlabel, n_fold):
    X = np.concatenate((X_train, X_unlabel), axis=0)
    y = np.concatenate((np.ones(len(X_train)),
                        -1*np.ones(len(X_unlabel))), axis=0)

    kf = KFold(n_splits=n_fold, shuffle=True, random_state=2020)
    score = np.zeros(len(X), np.float)
    ...
    prior_estimate = wrapper(score_unlabel, score_train, len(X_unlabel), len(X_train), 1)
...
def wrapper(X_mixture, X_component, num_mix, num_com, dim):
    ...
    print('Begin Estimating Prior.This may take a long time, please be patient:)')
...
def mpe(kernel, N, M, nu, epsilon=0.04, lambda_upper_bound=8.):
    ...
    while lambda_right - lambda_left > epsilon:
        # print('Left-Right: {:.2f}, epsilon: {:.2f}'.format(lambda_right - lambda_left, epsilon))
...
def get_distance_curve(
    ...
    for lambda_value in lambda_values:
        ...
        prev_soln = solution
        if distance_sqd < 0:
            print("[Warning] distance is {:.4f}".format(distance_sqd))
            distance_sqd = 0
